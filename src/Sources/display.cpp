...
LRESULT CALLBACK WinProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT:
    {
        ...
        Memory* memory = Display::getInstance().getMemory();
        if (memory) {
            ...
            graphics.FillRectangle(&scrollbarTrack,
                scrollbarX, memoryViewY, scrollbarWidth, memoryViewHeight);

            graphics.FillRectangle(&scrollbarThumb,
                scrollbarX, scrollbarY, scrollbarWidth, scrollbarHeight);

            SolidBrush headerBrush(Color(255, 200, 50));
            graphics.DrawString(L"MEMORY", -1, &font,
                PointF(memoryViewX, memoryViewY - 24), &headerBrush);
        }
